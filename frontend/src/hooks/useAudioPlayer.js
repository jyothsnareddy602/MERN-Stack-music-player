...
// Reducer function to manage audio state transitions
...
const useAudioPlayer = (songs) => {
    ...
    const previousVolumeRef = useRef(1); // store last non-zero volume
    ...
    // Audio event handler: update current time
    ...
    // Audio event handler: metadata loaded
    ...
    // Audio event handler: song ended
    const handleEnded = () => {
        const audio = audioRef.current;
        if (!audio) return;
